      ...
      startup: function() {
        ...
        if (this.map.itemId) {
          _estoLayerList = this;
            ...
            switch(_estoLayerList.appConfig.perfil) {
              case '1':
              array.forEach(operLayerInfos._layerInfos, function(layer) {
                if ( _estoLayerList.config.configLevels[_estoLayerList.appConfig.perfil].includes(layer.title)){
                  layer.title = "Outside"
                }
              });
              break;
