...
define([
  ...
  function(declare, lang, array, html, when, on, aspect, query, keys, Deferred, all, focusUtil,
    ...
    FeatureLayer, InfoTemplate, PopupTemplate, esriLang, Point, coordinateFormatter, SpatialReference, FeatureQuery,
    ...
    return declare([BaseWidget], {
      ...
      modificarInfoTemplate: function (searchSouces) {
        for (let index = 0; index < searchSouces.length; index++) {
          if (searchSouces[index].name === "Elemento Vulnerable Punto") {
            searchSouces[index].infoTemplate = new InfoTemplate(
              this.config.sources[index].name,
              `<span style='font-weight:bold;color:black;'>${this.config.sources[index].name}`+"</span></br>"+
              "<hr/>"+
              "<span style='font-weight:bold;'>ID Elemento Vulnerable Punto:</span> ${ID_ELEMENTOVULNERABLEP}</br>"+
              "<span style='font-weight:bold;'>ID Plan Director:</span> ${ID_PLANDIRECTOR}</br>"+
              "<span style='font-weight:bold;'>Nombre:</span> ${NOMBRE}</br>" +
              "<span style='font-weight:bold;'>Descripción:</span> ${DESCRIPCION}</br>" +
              "<span style='font-weight:bold;'>Tipo Vía:</span> ${DIRTIPO}</br>" +
              "<span style='font-weight:bold;'>Nombre Vía:</span> ${DIRNOMBRE}</br>" +
              "<span style='font-weight:bold;'>Número Vía:</span> ${DIRNUMERO}</br>" +
              "<span style='font-weight:bold;'>Calificador:</span> ${DIRCALIFICADOR}</br>" +
              "<span style='font-weight:bold;'>Población:</span> ${DIRPOBLACION}</br>" +
              "<span style='font-weight:bold;'>Distrito:</span> ${DISTRITO}</br>" +
              "<span style='font-weight:bold;'>Fecha Inicio:</span> ${FECHAINI}</br>" +
              "<span style='font-weight:bold;'>Fecha Fin:</span> ${FECHAFIN}</br>" +
              "<span style='font-weight:bold;'>Utmx:</span> ${UTMX}</br>" +
              "<span style='font-weight:bold;'>Utmy:</span> ${UTMY}</br>" +
              "<span style='font-weight:bold;'>Aforo:</span> ${AFORO}</br>" +
              "<span style='font-weight:bold;'>Telefono:</span> ${TELCONTACTO}</br>"
              );
          }
          if (searchSouces[index].name === "Localización") {
            searchSouces[index].infoTemplate = new InfoTemplate(
              this.config.sources[index].name,
              `<span style='font-weight:bold;color:black;'>${this.config.sources[index].name}`+"</span></br>"+
              "<hr/>"+
              "<span style='font-weight:bold;'>Id Localización:</span> ${ID_Localizacion}</br>"+
              "<span style='font-weight:bold;'>Id Elemento Vulnerable Punto:</span> ${ID_ElementoVulnerableP}</br>"+
              "<span style='font-weight:bold;'>Nombre:</span> ${Nombre}</br>" +
              "<span style='font-weight:bold;'>Descripción:</span> ${Descripcion}</br>" +
              "<span style='font-weight:bold;'>Fecha Baja:</span> ${FechaBaja}</br>" 
            );
          }

          if (searchSouces[index].name === "Elemento Vulnerable Área") {
          }
        }
        console.log(searchSouces)
        return searchSouces;
      },
